...
static IRAM_ATTR esp_err_t _iot_set_fade_with_time(ledc_mode_t speed_mode, ledc_channel_t channel, uint32_t target_duty, int max_fade_time_ms)
{
    ...
    uint32_t timer_source_clk = 1; // original: LEDC.timer_group[speed_mode].timer[g_light_config->timer_num].conf.tick_sel;
...
static IRAM_ATTR void fade_timercb(void *para)
{
    ...
    if (HW_TIMER_GROUP == TIMER_GROUP_0) {
        ...
        uint32_t intr_status = TIMERG0.int_st_timers.val; // original: uint32_t intr_status = TIMERG0.int_st.val;
...
#elif CONFIG_IDF_TARGET_ESP32S2 || CONFIG_IDF_TARGET_ESP32C3
            TIMERG0.int_clr_timers.t0_int_clr = 1; // original: TIMERG0.int_clr.t0 = 1;
#endif
        } 
        ...
        TIMERG0.hw_timer[timer_idx].config.tx_alarm_en = TIMER_ALARM_EN; //original: TIMERG0.hw_timer[timer_idx].config.alarm_en = TIMER_ALARM_EN;
    } 
#ifndef CONFIG_IDF_TARGET_ESP32C3
