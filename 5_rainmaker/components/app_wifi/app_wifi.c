...
#include <network_provisioning/manager.h> //original: #include <wifi_provisioning/manager.h>
#ifdef CONFIG_APP_WIFI_PROV_TRANSPORT_BLE
#include <network_provisioning/scheme_ble.h>// original: #include <wifi_provisioning/scheme_ble.h>
...
static void app_wifi_print_qr(const char *name, const char *pop, const char *transport)
{
    ...
    // original: qrcode_display(payload);
    esp_qrcode_config_t cfg = ESP_QRCODE_CONFIG_DEFAULT();
	esp_err_t ret = esp_qrcode_generate(&cfg, payload);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to generate QR code (err: %s).", esp_err_to_name(ret));
    }  
...
{
    if (event_base == NETWORK_PROV_EVENT) {// original: if (event_base == WIFI_PROV_EVENT) {
        switch (event_id) {
            case NETWORK_PROV_START:// original: case WIFI_PROV_START:
                ESP_LOGI(TAG, "Provisioning started");
                break;
            case NETWORK_PROV_WIFI_CRED_RECV: { // original: case WIFI_PROV_CRED_RECV: {
            ...
            case NETWORK_PROV_WIFI_CRED_FAIL: {// original: case WIFI_PROV_CRED_FAIL: {
                network_prov_wifi_sta_fail_reason_t *reason = (network_prov_wifi_sta_fail_reason_t *)event_data;// original: wifi_prov_sta_fail_reason_t *reason = (wifi_prov_sta_fail_reason_t *)event_data;
                ESP_LOGE(TAG, "Provisioning failed!\n\tReason : %s"
                         "\n\tPlease reset to factory and retry provisioning",
                         (*reason == NETWORK_PROV_WIFI_STA_AUTH_ERROR) ?// original: (*reason == WIFI_PROV_STA_AUTH_ERROR) ?
                         "Wi-Fi station authentication failed" : "Wi-Fi access-point not found");
                break;
            }
            case NETWORK_PROV_WIFI_CRED_SUCCESS:// original: case WIFI_PROV_CRED_SUCCESS:
                ESP_LOGI(TAG, "Provisioning successful");
                break;
            case NETWORK_PROV_END: // original: case WIFI_PROV_END:
                /* De-initialize manager once provisioning is finished */
                network_prov_mgr_deinit(); // original: wifi_prov_mgr_deinit();
                break;
            default:
                break;
        }
...
void app_wifi_init(void)
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_register(NETWORK_PROV_EVENT, ESP_EVENT_ANY_ID, &event_handler, NULL));// original: ESP_ERROR_CHECK(esp_event_handler_register(WIFI_PROV_EVENT, ESP_EVENT_ANY_ID, &event_handler, NULL));
...
esp_err_t app_wifi_start(app_wifi_pop_type_t pop_type)
{
    ...
    network_prov_mgr_config_t config = { //original: wifi_prov_mgr_config_t config = {
        /* What is the Provisioning Scheme that we want ?
         * wifi_prov_scheme_softap or wifi_prov_scheme_ble */
#ifdef CONFIG_APP_WIFI_PROV_TRANSPORT_BLE
        .scheme = network_prov_scheme_ble,// original: .scheme = wifi_prov_scheme_ble,
#else /* CONFIG_APP_WIFI_PROV_TRANSPORT_SOFTAP */
...
#ifdef CONFIG_APP_WIFI_PROV_TRANSPORT_BLE
        .scheme_event_handler = NETWORK_PROV_SCHEME_BLE_EVENT_HANDLER_FREE_BTDM// original: .scheme_event_handler = WIFI_PROV_SCHEME_BLE_EVENT_HANDLER_FREE_BTDM
#else /* CONFIG_APP_WIFI_PROV_TRANSPORT_SOFTAP */
    ...
    ESP_ERROR_CHECK(network_prov_mgr_init(config)); // original: ESP_ERROR_CHECK(wifi_prov_mgr_init(config));

    bool provisioned = false;
    /* Let's find out if the device is provisioned */
    ESP_ERROR_CHECK(network_prov_mgr_is_wifi_provisioned(&provisioned));// original: ESP_ERROR_CHECK(wifi_prov_mgr_is_provisioned(&provisioned));
    ...
    if (!provisioned) {
         ...
         */
        network_prov_security_t security = NETWORK_PROV_SECURITY_1;// original: wifi_prov_security_t security = WIFI_PROV_SECURITY_1;
        ...
        err = network_prov_scheme_ble_set_service_uuid(custom_service_uuid);// original: err = wifi_prov_scheme_ble_set_service_uuid(custom_service_uuid);
        if (err != ESP_OK) {
        ...
        ESP_ERROR_CHECK(network_prov_mgr_start_provisioning(security, pop, service_name, service_key));// original: ESP_ERROR_CHECK(wifi_prov_mgr_start_provisioning(security, pop, service_name, service_key));
